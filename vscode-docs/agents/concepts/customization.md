...
This article explains how customizations change an agent, how the customization types differ, and how you can combine them. To create and manage customizations, see [Customize agent behavior in {% data variables.product.prodname_vscode %}](../../agent-customization/overview.md).
...
* [Customize AI for your project](../guides/customize-copilot-guide.md)
...
* **Provide context**: [instructions](../../agent-customization/custom-instructions.md) describe coding standards, architecture decisions, and other rules the agent should follow.
* **Define repeatable work**: [agent skills](../../agent-customization/agent-skills.md) and [prompt files](../../agent-customization/prompt-files.md) package task-specific guidance so you don't have to describe the same process in every conversation.
* **Configure a role**: [custom agents](../../agent-customization/custom-agents.md) combine instructions, tools, and a language model into a specialized agent persona.
* **Add capabilities**: [MCP servers](../../agent-customization/mcp-servers.md) give the agent tools for interacting with external systems such as databases, browsers, and APIs.
* **Enforce actions**: [hooks](../../agent-customization/hooks.md) run commands at specific points in the agent loop, independent of whether the model chooses to run them.
* **Adopt a ready-made setup**: [agent plugins](../../agent-customization/agent-plugins.md) let you install multiple related customization types as one package.
...
|------|-----|---------|------------|
| Apply standards across a project | [Always-on instructions](../../agent-customization/custom-instructions.md) | Require a specific logging library and error-handling pattern | Automatically included in each request |
| Apply guidance to specific code or tasks | [File-based instructions](../../agent-customization/custom-instructions.md) | Apply React conventions when the agent works with `.tsx` files | Included when files match a pattern or the task matches the description |
| Teach the agent a workflow with supporting resources | [Agent skills](../../agent-customization/agent-skills.md) | Create a service from instructions, template files, and a setup script | The agent loads the skill when the task matches, or you invoke it directly |
| Run a saved task on demand | [Prompt files](../../agent-customization/prompt-files.md) | Scaffold a React component | You invoke the prompt as a slash command |
| Use a specialized role and tool configuration | [Custom agents](../../agent-customization/custom-agents.md) | Review code with read-only tools | You select the agent, use it as a subagent, or another agent delegates to it |
| Connect to an external system | [MCP](../../agent-customization/mcp-servers.md) | Query a database or update an issue | The agent calls an MCP tool when the task requires it |
| Run code at a lifecycle event | [Hooks](../../agent-customization/hooks.md) | Run a formatter after a file edit or block a risky command | {% data variables.product.prodname_vscode_shortname %} runs the hook when the configured event occurs |
| Install a packaged customization setup | [Agent plugins](../../agent-customization/agent-plugins.md) | Add a testing workflow with a skill, agent, hooks, and MCP server | Each bundled customization follows its own activation rules |
