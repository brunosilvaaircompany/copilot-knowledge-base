...
For an overview of AI workflows in {% data variables.product.prodname_vscode_shortname %} and where to work with agents, see [Build with AI in {% data variables.product.prodname_vscode_shortname %}](../overview.md).
...
1. **Context and reasoning:** {% data variables.product.prodname_vscode_shortname %} assembles relevant [context](context.md), such as files and conversation history. A [language model](language-models.md) reasons over that information and determines the next action.
1. **Tool action:** The agent uses [tools](tools.md) to gather more information or act on your environment, such as reading a file, editing code, or running a command.
...
The repeated reasoning, action, and validation steps form the **agent loop**. A [session](sessions.md) keeps the conversation and task state together. An [agent harness](agent-harnesses.md) coordinates the loop, while the execution environment determines where tools run and code changes are made.
...
Learn more about [trust and safety controls](trust-and-safety.md).
...
Sessions are independent, can run in parallel, and are shared across the [{% data variables.copilot.chat_view %}](../run/chat-view.md) and the [{% data variables.copilot.agents_window %}](../run/agents-window.md). A session can run on your machine or a remote host, and you can hand it off to another agent. Learn more about [sessions and handoff](sessions.md).
...
The execution environment is where the harness runs tools and changes code. Depending on the harness and task, this can be your machine, cloud infrastructure, or a remote machine. Learn more about [agent harnesses and execution environments](agent-harnesses.md).
...
The Plan agent uses read-only tools while preparing the plan. After you review and approve the approach, you can hand off the plan to an implementation agent. Learn more about [planning with agents](../run/planning.md).
...
Subagents are useful when independent research would add large amounts of intermediate information to the main agent's [context window](language-models.md#context-window). Each subagent works in a separate context window and returns a focused result.
...
Learn more about [using subagents](../run/subagents.md).
...
* A [**custom agent**](../../agent-customization/custom-agents.md) defines instructions, available tools, a language model, and optional handoffs for a specific workflow.
* [**Agent skills**](../../agent-customization/agent-skills.md) give an agent instructions and resources for a specialized task.
* [**Hooks**](../../agent-customization/hooks.md) run commands at specific points in the agent lifecycle.

Learn more about [customization concepts](customization.md).

## Related resources

* [Tools](tools.md)
* [Agent harnesses and execution environments](agent-harnesses.md)
