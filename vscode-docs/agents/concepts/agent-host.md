...
**NOTE:** The Agent Host and AHP are under active development, and new capabilities continue to roll out.
...
| MCP configuration | The Agent Host reads harness-agnostic MCP config from `.mcp.json` (workspace) and `~/.copilot/mcp-config.json` (user). It doesn't read `.vscode/mcp.json` directly, but {% data variables.product.prodname_vscode_shortname %} forwards servers you configure in {% data variables.product.prodname_vscode_shortname %} to the Agent Host, except servers that require interactive input (for example, `${input:...}` variables). Learn more about [configuring MCP servers](../../agent-customization/mcp-servers.md). |
...
* [Agents in {% data variables.product.prodname_vscode_shortname %}](agents.md)
* [Remote agent sessions](../run/remote-agent-sessions.md)
* [{% data variables.product.prodname_vscode_shortname %} Remote Development architecture](https://code.visualstudio.com/docs/remote/remote-overview)
