...
This article provides a reference for the MCP server configuration file format, related commands, and settings in {% data variables.product.prodname_vscode_shortname %}. For information about adding and managing MCP servers, see [Add and manage MCP servers](../../agent-customization/mcp-servers.md).
...
**NOTE:** When using Docker with stdio servers, don't use the detach option (`-d`). The server must run in the foreground to communicate with {% data variables.product.prodname_vscode_shortname %}.
...
**NOTE:** When sandboxing is enabled, tool confirmations are auto-approved because the server runs in a controlled environment.
...
**NOTE:** When `oauth` is configured, {% data variables.product.prodname_vscode_shortname %} handles the OAuth flow automatically. A browser window opens for authorization on the first connection to the server.
...
When you reference an input variable using `${input:variable-id}`, {% data variables.product.prodname_vscode_shortname %} prompts you for the value when the server starts for the first time. The value is then securely stored for subsequent use. Learn more about [input variables](https://code.visualstudio.com/docs/reference/variables-reference#input-variables) in {% data variables.product.prodname_vscode_shortname %}.
...
For a full list of {% data variables.product.prodname_vscode_shortname %} AI settings, see the [AI Settings Reference](ai-settings.md). The following settings are specific to MCP servers.
