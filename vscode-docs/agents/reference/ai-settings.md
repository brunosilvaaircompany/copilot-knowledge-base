...
| Setting and Description | Default |
|------------------------|---------------|
| `setting(chat.agentMerge.enabled)` _(Experimental)_<br/>Enable [Agent Merge](../run/agents-window.md#finish-a-pull-request-with-agent-merge) and its commands. Agent Merge monitors a session's pull request and can ask the agent to address selected blockers. | `false` |
