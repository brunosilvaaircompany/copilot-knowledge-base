...
Look up common tool names and context references for chat prompts in {% data variables.product.prodname_vscode %}. For instructions on selecting tools, invoking them, and managing approvals, see [Use tools with agents](../run/tools.md).
...
Referencing a tool does not bypass its [approval and permission controls](../run/approvals.md).
...
| `#execute/createAndRunTask` | Create and run a [workspace task](https://code.visualstudio.com/docs/debugtest/tasks). |
...
| Reference | Purpose |
|---|---|
| `#browser` (tool set) | Navigate, inspect, and interact with pages using [browser tools](../run/browser-tools.md). |
...
| Reference | Purpose |
|---|---|
| `#agent` (tool set) | Delegate tasks to [subagents](../run/subagents.md). |
...
For tools that coordinate work across Agent Host sessions, see [session orchestration](../run/sessions/manage-sessions.md#orchestrate-sessions-from-agent-host-sessions).
...
Use **Add Context** or drag items into chat to attach other context, such as images, errors, or GitHub issues. See [Add context to chat](../../chat/copilot-chat-context.md) for supported sources and attachment methods.

## Related resources

* [Understand tools in the agent loop](../concepts/tools.md).
