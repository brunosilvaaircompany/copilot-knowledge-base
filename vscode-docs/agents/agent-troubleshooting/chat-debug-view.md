...
**NOTE:** The Agent Debug Log panel is currently in preview.

The Agent Debug Log panel is the primary tool for understanding what happens when you send a prompt. It shows a chronological event log of agent interactions during a chat session, making it especially useful when debugging [custom agents](../../agent-customization/custom-agents.md) and orchestrated subagent workflows.
...
* **[Cache Explorer](cache-explorer.md)**: a side-by-side diff of consecutive model requests that helps diagnose prompt-cache misses.

**NOTE:** The Agent Debug Logs panel now shows both current and historical sessions. Logs are persisted locally on disk allowing you to view historical sessions.
...
From the Summary view, you can select **View Logs**, **Agent Flow Chart**, or **[Cache Explorer](cache-explorer.md)** to switch to one of those views.
...
To learn how to open and read the Cache Explorer and how to improve your cache hit rate, see [Diagnose prompt caching with the Cache Explorer](cache-explorer.md).
...
**NOTE:** The `/troubleshoot` command requires the `setting(github.copilot.chat.fileLogging.enabled)` setting to be enabled.
...
You can expand each section to see the full details. This is particularly useful when [using agents](../concepts/agents.md), where multiple tools might be invoked as part of a single request.
...
1. Open the Chat Debug view and check the **Context** section to verify that workspace files appear in the context. If they don't, check that [workspace indexing](../reference/workspace-context.md) is active.
1. Try adding explicit `#`-mentions (such as `#file` or `#codebase`) to ensure the right files are included. Learn more about [managing context](../../chat/copilot-chat-context.md).
...
1. A full context window might cause the model to truncate its response. Start a [new chat session](../run/sessions/manage-sessions.md) to reset the context.
...
1. See [Debug chat interactions](troubleshooting.md#debug-chat-interactions) for more troubleshooting options.

## Related resources

* [Chat overview](../../chat/chat-overview.md)
* [Add context to chat](../../chat/copilot-chat-context.md)
* [Troubleshoot AI in {% data variables.product.prodname_vscode_shortname %}](troubleshooting.md)
* [Security considerations for using AI in {% data variables.product.prodname_vscode_shortname %}](../run/security.md)
