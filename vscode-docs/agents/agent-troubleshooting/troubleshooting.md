...
|---------|------------|-----------|
| Chat or agents are unavailable | Check [chat and agent requirements](faq.md#chat-features-arent-working-for-me). | Confirm your account, plan, settings, and organization policies. |
| Inline suggestions do not appear | Check the [inline suggestion requirements](faq.md#inline-suggestions-are-not-working-in-the-editor). | Inspect the Copilot logs if the requirements are met. |
...
1. Confirm your access method. Sign in with the GitHub account that has your Copilot plan, or select a configured bring-your-own-key model for supported chat features. Some features require the GitHub Copilot service. For details, see [GitHub Copilot subscription questions](faq.md#github-copilot-subscription).
...
1. Check whether a workspace setting or organization policy turns off the affected feature. For agent availability, see [Agents are not available in chat](faq.md#agents-are-not-available-in-chat).
...
For file locations, applicability rules, and settings, see [Use custom instructions](../../agent-customization/custom-instructions.md). To verify the exact instructions sent to the model, use the [Chat Debug view](chat-debug-view.md#chat-debug-view).
...
* For sessions that run on the [Agent Host](../concepts/agent-host.md), enable the experimental `setting(chat.agentHost.agentDebugLog.enabled)` setting before the activity that you want to inspect. Capture is not retroactive.
...
For detailed procedures and scenario-based guidance, see [Debug chat interactions](chat-debug-view.md).
...
For required domains and network-specific guidance, see [Network and firewall configuration for Copilot](faq.md#network-and-firewall-configuration-for-copilot).
...
If the server is running but the model does not invoke its tool, follow [An MCP tool is not being invoked](chat-debug-view.md#an-mcp-tool-is-not-being-invoked).

For setup and server-specific checks, see [Configure and debug MCP servers](../../agent-customization/mcp-servers.md#troubleshoot-and-debug-mcp-servers).
...
* [Debug chat interactions](chat-debug-view.md)
* [GitHub Copilot frequently asked questions](faq.md)
