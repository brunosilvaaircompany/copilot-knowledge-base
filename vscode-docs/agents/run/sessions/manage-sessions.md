...
* Use the [{% data variables.copilot.agents_window %}](../agents-window.md#start-an-agent-session) to start and monitor sessions across workspaces, attach related projects or GitHub items, or start from an existing pull request.
...
**NOTE:** Side chats are available only in the {% data variables.copilot.agents_window %} for Copilot and Claude sessions. They aren't available for Codex sessions or in the {% data variables.copilot.chat_view %}.
...
For multi-chat [Copilot](../agent-harnesses.md#use-the-copilot-harness) sessions in the {% data variables.copilot.agents_window %}, the fork opens as a peer chat in the same session. The peer chat gets an automatically generated title and runs independently from sibling chats.
...
* [Use chat in {% data variables.product.prodname_vscode_shortname %}](../../../chat/chat-overview.md)
