...
A [session](../../concepts/sessions.md) is the unit of work with an agent in Visual Studio Code. It includes the sequence of prompts and responses, relevant context, and any files or resources associated with the task. This article describes how to create, organize, and manage sessions in both the [Chat view](../chat-view.md) and the [Agents window](../agents-window.md).
...
The [Agents window](../agents-window.md) is a dedicated window for orchestrating agents across multiple projects from a single place. Chat is your primary interface where you assign high-level tasks to agents. The Agents window is optimized for **agents-first workflows**.
...
1. Optionally, select additional configuration options for the session:
    ...
    * **Language model**: determines which AI model powers the conversation. Learn more about [language models in VS Code](../../../agent-customization/language-models.md).
...
The [Chat view](../chat-view.md) is a chat panel that sits in the sidebar alongside your workspace editor tabs. Agents assist you with coding tasks, while you have full access to VS Code's rich coding experience. The Chat view is optimized for **code-first workflows**.
...
How context changes across turns also affects prompt caching. Stable context lets the model provider reuse tokens from earlier requests, which lowers cost and latency. Use the [Cache Explorer](../../agent-troubleshooting/cache-explorer.md) to check your cache hit rate.
...
Learn more about [AI credit consumption](../../guides/optimize-usage.md).
...
**NOTE:** Side chats are available only in the Agents window for Copilot and Claude sessions. They aren't available for Codex sessions or in the Chat view.
...
**NOTE:** Extension developers can learn how to integrate with the sessions view by using the proposed API [`chatSessionsProvider`](https://github.com/microsoft/vscode/blob/main/src/vscode-dts/vscode.proposed.chatSessionsProvider.d.ts). The API is currently in a proposed state and subject to change.
...
To permanently delete a session, right-click the session in the sessions list and select **Delete**. Deleting a session removes it permanently and can't be undone. For [Copilot sessions](../agent-harnesses.md#copilot), deleting the session also removes any associated worktrees created for that session.
...
For multi-chat [Copilot](../agent-harnesses.md#use-the-copilot-harness) sessions in the Agents window, the fork opens as a peer chat in the same session. The peer chat gets an automatically generated title and runs independently from sibling chats.
...
**TIP:** A forked session inherits the conversation history of the original, which preserves the prompt cache and reduces cost on the next request. Use the [Cache Explorer](../../agent-troubleshooting/cache-explorer.md) to compare cache hit rates across sessions.

## Orchestrate sessions from agent host sessions

In agent host sessions, such as [Copilot](../agent-harnesses.md#use-the-copilot-harness) and Claude, agents can use built-in session-management tools to coordinate work across multiple sessions and chats. These tools are also available for Codex sessions when Codex [runs on the Agent Host](../../concepts/agent-host.md#agents-on-the-agent-host).
...
* [Use chat in VS Code](../../../chat/chat-overview.md)
* [Agents overview](../../overview.md)
* [Add context to chat](../../../chat/copilot-chat-context.md)
* [Best practices for using AI](../../best-practices.md)
