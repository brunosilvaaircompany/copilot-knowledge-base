...
For the relationship between harnesses, language models, agent roles, and execution environments, see [Agent harnesses](../concepts/agent-harnesses.md).
...
Learn more about [creating and managing sessions](sessions/manage-sessions.md).
...
|--------|------------------|---------------|----------------|
| **New Worktree** | A new branch and [worktree](https://code.visualstudio.com/docs/sourcecontrol/branches-worktrees#understanding-worktrees) | Parallel tasks that should not modify your active workspace | Starts from committed Git state and requires you to integrate the result |
...
When you [start a session in the Agents window](agents-window.md#start-an-agent-session), select **New Worktree** and choose the base branch to isolate the session. If you leave **New Worktree** unselected, the agent works directly on the code in the workspace. Sessions that you start in the Chat view always use the current workspace.
...
The Local harness runs interactively in the VS Code extension host and works directly in your current workspace. It can use VS Code built-in tools, extension-provided tools, MCP servers, and the models configured in VS Code, including [bring your own key models](../../agent-customization/language-models.md#bring-your-own-language-model-key).
...
* **Plan**: researches a task and creates a structured implementation plan before code changes. Learn more about [planning with agents](planning.md).

You can switch roles during a session from the agent picker. For specialized workflows, [create a custom agent](../../agent-customization/custom-agents.md).
...
The Copilot harness is powered by the [Copilot SDK](https://www.npmjs.com/package/@github/copilot-sdk) and runs on the [Agent Host](../concepts/agent-host.md). The Agent Host owns the session independently of the windows that display it, so a session can continue in the background, appear in multiple windows, and contain multiple chats.
...
Copilot supports slash commands for common session operations. Enter `/` in the chat input to view available commands. For example, use `/compact` to reduce conversation context, `/research` to start deep research, or `/yolo` and `/autoApprove` to control [automatic tool approval](approvals.md#frequently-asked-questions).

### Permissions and approvals

The available [permission levels](approvals.md#permission-levels) depend on the isolation mode:
...
Because Copilot sessions run on the Agent Host, **Autopilot** is an [agent mode](approvals.md#how-autopilot-works) rather than a permission level.
...
To use a [custom agent](../../agent-customization/custom-agents.md) in a Copilot session:
...
**NOTE:** The research agent is in preview and is available only in Copilot sessions in VS Code Insiders.
...
For research that feeds into an implementation plan, use the [Plan agent](planning.md). For focused research that returns results to the current conversation, use [subagents](subagents.md).
...
You can also select a GitHub repository when you [start a session in the Agents window](agents-window.md#start-an-agent-session), or [hand off an existing session](#hand-off-a-session) to a cloud harness. In a Copilot session, enter `/delegate` to continue the task in the cloud.
...
**TIP:** In Copilot CLI, enter `/delegate` to continue the work with a cloud agent.

### Hand off a plan to implementation

The [Plan agent](planning.md) focuses on researching a task and creating a plan without changing code. After you review the plan:
...
| **Fork a session** | Creates an independent session from a point in the conversation. Learn more about [forking sessions](sessions/manage-sessions.md#fork-a-chat-session). |
| **Switch surfaces** | Opens the same session in the [Chat view](chat-view.md) or [Agents window](agents-window.md) without changing its target or context. |

For background on how handoff works, see [Sessions and handoff](../concepts/sessions.md#hand-off-a-session).

## Related resources

* [Agent harness concepts](../concepts/agent-harnesses.md)
* [Manage agent sessions](sessions/manage-sessions.md)
* [Approvals and permissions](approvals.md)
* [Agent Host architecture](../concepts/agent-host.md)
