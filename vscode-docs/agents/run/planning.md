...
For an end-to-end example, follow the guide to [add a feature to an existing project](../guides/add-a-feature.md), from defining requirements to reviewing the tested change.
...
Open your project folder in {% data variables.product.prodname_vscode_shortname %} and [set up {% data variables.product.prodname_copilot %}](https://code.visualstudio.com/docs/setup/copilot) before you begin.

The following steps use the **{% data variables.product.prodname_copilot_short %}** session target. Learn more about [choosing an agent harness](agent-harnesses.md).
...
**NOTE:** Approving a plan and approving tool actions are separate decisions. Review the session's [permissions](approvals.md#permission-levels) before starting implementation. In a {% data variables.product.prodname_copilot_short %} session, Autopilot controls how the agent continues working, while the selected approval level controls which actions need confirmation.
...
In a {% data variables.product.prodname_copilot_short %} session, use the [language model picker](../../agent-customization/language-models.md#change-the-model-for-chat) to choose the planning model. To use a different model for a separate implementation request:
...
For model-selection guidance, see [Optimize AI credit usage](../guides/optimize-usage.md).

For a repeatable planning workflow, define a [custom agent](../../agent-customization/custom-agents.md) with instructions such as architectural guidelines or required planning deliverables.
...
| Goal | Action |
|------|--------|
| Read the full plan | Run **Chat: Show Memory Files** from the Command Palette (`kb(workbench.action.showCommands)`) and select the session's `plan.md`. The plan is stored in [session memory](memory.md#session-memory) at `/memories/session/plan.md`, not as a project file. |
...
| Hand off to another destination | Open the dropdown next to **Start Implementation** and select an available **Continue in** destination. The dropdown appears only when supported destinations are available. Learn more about [session handoffs](agent-harnesses.md#hand-off-a-session). |
...
* [Review AI-generated code edits](review-code-edits.md)
* [Write effective prompts](../guides/prompt-engineering-guide.md)
