...
* Set up and open the [{% data variables.copilot.agents_window %}](agents-window.md#prerequisites) with an available agent.
...
1. Choose an available agent and review **Session configuration**, including the model and permission options. For a Git workspace, you can select **New Worktree** and a base branch when the agent supports isolation. Learn how to [choose an agent and code isolation](agent-harnesses.md).
...
> [!CAUTION]
> An automation can read files, run commands, and make changes according to its agent's permissions. Review the [permission level](approvals.md#permission-levels) before scheduling unattended work. Saving an automation doesn't bypass organization policies or guarantee that future runs won't need your approval.
...
* **Templates from Plugins** contains templates from enabled [Agent Plugins](../../agent-customization/agent-plugins.md#automations-in-plugins). Each template shows a **Plugin** badge and its source plugin.
...
* [Discover and manage Agent Plugins](../../agent-customization/agent-plugins.md)
