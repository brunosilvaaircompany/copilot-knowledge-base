...
To control which calls require approval and how much autonomy the agent has, see [Manage approvals and permissions](approvals.md).
...
The `#` menu also contains context items, such as files, folders, and editor selections. Context items provide information for the request, while tools give the agent capabilities to gather information or take actions. Learn more about [adding context to chat prompts](../../chat/chat-overview.md#add-context-to-your-prompts).
...
1. Select or deselect tools to control which ones are available to the Copilot harness. These choices persist in the active [user profile](https://code.visualstudio.com/docs/configure/profiles).




For either session type, limit the selection to tools that are relevant to your request. Add capabilities by [installing MCP servers](../../agent-customization/mcp-servers.md) or [extensions](https://code.visualstudio.com/docs/configure/extensions/extensions) that contribute tools.

When you customize chat with [prompt files](../../agent-customization/prompt-files.md) or [custom agents](../../agent-customization/custom-agents.md), you can specify which tools are available for a workflow. Learn more about the [tool list priority order](../../agent-customization/custom-agents.md#tool-list-priority).
...
Tool availability is separate from tool approval. Enabling a tool makes it available to the agent, but doesn't bypass the configured [approval and permission controls](approvals.md).
