...
**NOTE:** This option is currently available for quick chats that use the Copilot harness. The target must be a local folder. [Worktree isolation](agent-harnesses.md#choose-code-isolation) requires a local Git repository with at least one commit.
