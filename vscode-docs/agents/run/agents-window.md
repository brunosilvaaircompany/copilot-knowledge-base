...
**NOTE:** You can hide the **Open in Agents** button by right-clicking it in the title bar and selecting **Hide 'Open in Agents'**. You can still open the {% data variables.copilot.agents_window %} at any time from the Command Palette or command line.
...
The {% data variables.copilot.agents_window %} lets you start a new session for any of your workspaces, including local folders, GitHub repositories, and [remote workspaces](remote-agent-sessions.md). You can also start a [quick chat](#start-a-quick-chat) that isn't associated with a workspace to ask a question or run a task that doesn't belong to a specific project.
