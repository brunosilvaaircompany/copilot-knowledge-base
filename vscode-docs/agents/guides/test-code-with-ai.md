...
A testing extension is optional for running tests in the terminal. To discover and run tests in the Test Explorer, install an appropriate [testing extension](https://code.visualstudio.com/docs/debugtest/testing#extensions-for-testing).
...
**TIP:** To reuse your testing conventions and safeguards across sessions, see [Make the workflow repeatable](#make-the-workflow-repeatable).

## Define the behavior to test

Choose a small scope and describe its expected behavior. A focused set of tests is easier to review for missing cases and incorrect assumptions. Include the source file and a representative existing test as [context in your prompt](../../chat/chat-overview.md#add-context-to-your-prompts) so the agent can follow the project's patterns.
...
The agent can use available [tools](../run/tools.md) to run tests and inspect their output. Review commands before approving execution. If execution is blocked or the required environment is unavailable, run the command yourself and provide the failure output. Treat tests that weren't run as unverified.
...
Before keeping the changes, [review the diff](../run/review-code-edits.md) and test output:
...
If your runner supports [test coverage](https://code.visualstudio.com/docs/debugtest/testing#test-coverage), use it to find untested branches and decide which behavior to cover next. Coverage shows which code executed, not whether the assertions are correct. A passing suite, even with high coverage, doesn't prove that the implementation is correct.

## Make the workflow repeatable

Save the test commands, framework, mocking conventions, and test-quality safeguards in your project's [custom instructions](../../agent-customization/custom-instructions.md). This helps the agent follow the same guidance across sessions without you repeating it in every prompt. Keep the instructions in version control to share them with your team.
...
* Follow a complete workflow to [fix an API bug with an agent](fix-a-bug-with-agents.md).
* Set up a test-first workflow with [TDD custom agents and handoffs](test-driven-development-guide.md).
* Try [testing web apps with browser agent tools](browser-agent-testing-guide.md).
