...
* Choose a small feature with an observable result. If you're unfamiliar with the implementation, [explore the codebase](explore-a-codebase.md) first.
* Preserve unrelated work and create a [development branch](https://code.visualstudio.com/docs/sourcecontrol/branches-worktrees) using your team's normal workflow.

For this guide, work in the current folder on that branch. Review the [recommended security baseline](../run/security.md#recommended-security-baseline) before letting an agent work in an existing project.
...
You can also use the dedicated [planning workflow](../run/planning.md) to create and refine the plan. An instruction to wait for approval doesn't change the session's tool permissions.
...
If the agent changes unrelated behavior, send a focused correction. For repeated failures or unwanted changes, use the [agent recovery guide](get-agent-back-on-track.md).
...
[Review the complete diff](../run/review-code-edits.md), including tests, documentation, and untracked files. Confirm that every change supports the approved feature and that no unrelated work is included.
...
* [Adapt agents to your project with instructions and skills](customize-copilot-guide.md).
* [Run independent tasks in separate worktrees](delegate-two-tasks.md).
