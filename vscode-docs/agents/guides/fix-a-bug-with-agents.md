...
**NOTE:** The sample API fits in one file and uses only built-in Node.js modules, so you can focus on the debugging workflow. The same process applies when request handling, application logic, and tests are spread across a larger project.
...
If the agent also changes the implementation, [restore the previous checkpoint](../run/review-code-edits.md#restore-a-checkpoint) and repeat the request with a narrower scope.
...
* Explore more ways to [test with GitHub Copilot](test-code-with-ai.md).
* Review how to [inspect, revise, and revert agent changes](../run/review-code-edits.md).
