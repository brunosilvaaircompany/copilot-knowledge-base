...
For a symbol rename or a supported method extraction, prefer the editor's [language-aware refactoring actions](https://code.visualstudio.com/docs/editing/refactoring). Use an agent when the task also needs investigation, design decisions, or coordinated edits that those actions don't cover.

## Prerequisites

* [Set up {% data variables.product.prodname_copilot %}](https://code.visualstudio.com/docs/setup/copilot).
...
* Preserve unrelated work and create a [development branch](https://code.visualstudio.com/docs/sourcecontrol/branches-worktrees) for the refactor.

Work in the current folder on that branch. If the existing behavior is unclear, [trace it through the codebase](explore-a-codebase.md) before changing its structure.
...
1. Open the [{% data variables.copilot.chat_view %}](../run/chat-view.md) and start a new chat.
...
If the behavior isn't sufficiently covered, [add tests for the existing code](test-code-with-ai.md) first:
...
Prompts guide the agent but don't enforce access restrictions. Review the session's [permissions and tool approvals](../run/approvals.md) before implementation.
...
[Review the complete diff](../run/review-code-edits.md) against the goal and preserved behavior. Check for unrelated formatting, dependency changes, weakened tests, or accidental changes to defaults and error handling.
...
If behavior changed unexpectedly, stop and investigate. Use your version-control baseline or the [agent recovery workflow](get-agent-back-on-track.md) to recover unwanted edits without discarding unrelated work.

You finish with a smaller or clearer implementation and evidence that the behavior you set out to preserve still holds.

## Next steps

* [Add new behavior as a separate feature](add-a-feature.md).
* [Set up a test-driven development workflow](test-driven-development-guide.md).
