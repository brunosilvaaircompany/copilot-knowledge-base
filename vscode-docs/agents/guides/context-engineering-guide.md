...
**TIP:** VS Code chat provides a [built-in plan agent](../run/planning.md) to help you create detailed implementation plans before starting complex coding tasks. If you don't want to create a custom planning workflow, you can use the plan agent to generate implementation plans quickly.
...
1. Create a `.github/copilot-instructions.md` [instructions file](../../agent-customization/custom-instructions.md#use-a-githubcopilot-instructionsmd-file) at the root of your repository.
...
With a [custom agent](../../agent-customization/custom-agents.md) for planning, you can create a dedicated persona with planning-specific guidelines and tools (for example, read-only access to the codebase). They can also capture specific workflows for brainstorming, researching, and collaborating for your project and team. For deep research that produces a thorough, well-cited report on a topic, use the built-in [research agent](../run/agent-harnesses.md#run-deep-research-with-the-research-agent) in Copilot sessions.
...
1. Create a planning [agent](../../agent-customization/custom-agents.md) `.github/agents/plan.agent.md`

    The planning agent defines a planning persona and instructs the agent not to perform implementation tasks, but to focus on creating the implementation plan. You can specify [handoffs](../../agent-customization/custom-agents.md#handoffs) to transition to an implementation agent after the plan is complete.
...
1. Optionally, create a [prompt file](../../agent-customization/prompt-files.md) `.github/prompts/plan.prompt.md` that invokes plan agent and instructs the agent to create an implementation plan from a provided feature request.
...
**TIP:** Use custom agents to define workflows that follow a multi-turn process with specific tools. Use them stand-alone or in combination with prompt files to add different variants and configurations of the same workflows.
...
1. For a more customized workflow, create a [custom agent](../../agent-customization/custom-agents.md) `.github/agents/implement.agent.md` specialized in implementing code based on a plan.
...
**TIP:** Get a fresh pair of agent eyes: create a new chat (`kb(workbench.action.chat.newChat)`) and ask the agent to review the code changes against the implementation plan. It can help identify any missed requirements or inconsistencies.
...
**Be mindful of credit consumption**: More context files, larger instruction sets, and complex agent chains all increase token usage and [AI credit](../concepts/language-models.md#ai-credits-and-model-costs) consumption. Start with concise context and expand only when needed. For more tips, see [optimize AI credit usage](optimize-usage.md).
...
**Handoffs between agents**: Use [handoffs](../../agent-customization/custom-agents.md#handoffs) to create guided transitions and implement end-to-end development workflows between planning, implementation, and review agents.
...
**Verify cache performance**: Use the [Cache Explorer](../agent-troubleshooting/cache-explorer.md) to check prompt cache hit rates and token usage. Good cache performance means your context setup is structured in a way that the model provider can reuse previous request prefixes, reducing latency and token costs.
...
**Over-engineering agent chains**: Deeply nested subagent workflows and excessive tool calls multiply token usage and [credit consumption](../concepts/language-models.md#ai-credits-and-model-costs). Keep agent chains as shallow as practical and limit tools to what each agent actually needs.
...
**For large projects**: Consider creating context hierarchies with project-wide, module-specific, and feature-specific context layers using [instructions files](../../agent-customization/custom-instructions.md).
...
* [Instructions files](../../agent-customization/custom-instructions.md)
* [Custom agents](../../agent-customization/custom-agents.md)
* [Prompt files](../../agent-customization/prompt-files.md)
