...
**NOTE:** For Agent Host sessions, such as Copilot sessions in the [{% data variables.copilot.agents_window %}](../agents/run/agents-window.md), enable `setting(chat.agentHost.byokModels.enabled)` to use BYOK models in those sessions. This setting is experimental and takes effect only after the agent host process is restarted.
