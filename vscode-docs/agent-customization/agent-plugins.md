...
Agent Plugins is an [open standard](https://agent-plugins.org/) for packaging [agent skills](agent-skills.md) and [MCP servers](mcp-servers.md) that works across multiple AI agents, including GitHub Copilot in VS Code, GitHub Copilot CLI, and the GitHub Copilot app.
Through its existing Copilot and Claude plugin formats, VS Code also supports client-specific plugin capabilities, including slash commands, [custom agents](custom-agents.md), and [hooks](hooks.md).
...
**NOTE:** Existing Copilot-format plugins that don't declare the Agent Plugins schema remain supported. For their manifest fields, see the [GitHub Copilot CLI plugin reference](https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-plugin-reference#pluginjson).
...
**NOTE:** Hooks are client-specific and are not a portable Agent Plugins 1.0 component type.
...
Plugin hooks use the same base format as [workspace hooks](hooks.md#hook-configuration-format). VS Code parses Claude Code hook configuration, including matcher syntax. Currently, VS Code ignores matcher values, so hooks run on every matching event.
...
* Use the [Agent Customizations editor](overview.md#use-the-agent-customizations-editor) to toggle a plugin's enabled state.
