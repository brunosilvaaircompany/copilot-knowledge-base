...
* [Customization concepts](../agents/concepts/customization.md)
...
* [Customize AI for your project](../agents/guides/customize-copilot-guide.md)
...
When you open the Customizations editor, the customizations are scoped to the selected [agent harness](../agents/concepts/agent-harnesses.md). Select the harness from the dropdown in the chat input before you open the editor to ensure the customizations apply to the correct context.
...
See the guides for [custom instructions](custom-instructions.md), [agent skills](agent-skills.md), [prompt files](prompt-files.md), [custom agents](custom-agents.md), [MCP servers](mcp-servers.md), [hooks](hooks.md), and [agent plugins](agent-plugins.md) for their file formats and configuration options.
...
If the result doesn't match your intent, make the customization more specific, resolve conflicting guidance, and repeat the task. For a complete project workflow, including how to review and share generated instructions, see [Configure AI for your codebase](../agents/guides/customize-copilot-guide.md).
...
* The parent repository folder is [trusted](https://code.visualstudio.com/docs/editing/workspaces/workspace-trust). {% data variables.product.prodname_vscode_shortname %} prompts you to trust the parent folder when the workspace is opened.

**NOTE:** The `setting(chat.useCustomizationsInParentRepositories)` setting is disabled by default.

## Migrate customizations

`feature(user-customization-migration)`

**NOTE:** Customization migration is available only in {% data variables.product.prodname_vscode_shortname %} Insiders.

[Agent Host](../agents/concepts/agent-host.md) sessions load customizations from supported folders and don't use some {% data variables.product.prodname_vscode_shortname %}-specific formats and locations. The Agent Customizations editor provides separate migrations for each type of incompatibility.
...
**IMPORTANT:** Prompt files are deprecated for Agent Host sessions and aren't loaded by Agent Host. They continue to work with the Local agent for now, but the Local agent will be removed in a future release. Convert prompt files to [agent skills](agent-skills.md) to keep them available.
...
The migrated files don't roam across devices through [Settings Sync](https://code.visualstudio.com/docs/configure/settings-sync). If you keep the original files in your profile user data, the original and migrated copies don't stay synchronized.
...
**NOTE:** The Chat Customizations Evaluations extension is published separately from {% data variables.product.prodname_vscode_shortname %}. Install it from the [{% data variables.product.prodname_vs_marketplace %}](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-chat-customizations-evaluations).
...
**TIP:** You can also start an analysis from chat with the `/analyze-prompt` slash command, which summarizes the diagnostics for the active customization file directly in the {% data variables.copilot.chat_view %}.
...
* [Customization concepts](../agents/concepts/customization.md)
* [Customize AI for your project guide](../agents/guides/customize-copilot-guide.md)
