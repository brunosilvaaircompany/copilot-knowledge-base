...
The [built-in agent roles](../agents/run/agent-harnesses.md#choose-a-built-in-agent-role) provide general-purpose configurations for chat in VS Code. For a more tailored chat experience, you can create your own custom agents.
...
You can reuse your custom agents with [Copilot and cloud harnesses](../agents/run/agent-harnesses.md), enabling you to run autonomous tasks with the same specialized configurations.
...
**IMPORTANT:** When [Agent Host](../agents/concepts/agent-host.md) is enabled, the agent reads user-level custom agents from `~/.copilot/agents` and not from VS Code profile user data. To use existing user-level custom agents with the Copilot agent, store them in `~/.copilot/agents`.
...
| `user-invocable`  | Optional boolean flag to control whether the agent appears in the agents dropdown in chat (default is `true`). Set to `false` to create agents that are only accessible as [subagents](../agents/run/subagents.md) or programmatically. |
...
| `hooks` (Preview)  | Optional hook commands scoped to this agent. Hooks defined here only run when this agent is active, either invoked by the user or as a subagent. Uses the same format as [hook configuration files](hooks.md#hook-configuration-format). Requires `setting(chat.useCustomAgentHooks)` to be enabled. |
...
**NOTE:** VS Code also detects `.md` files in the `.claude/agents` folder, following the [Claude sub-agents format](https://code.claude.com/docs/en/sub-agents). This enables you to use the same agent definitions across VS Code and Claude Code.

## Create a custom agent

You can create a custom agent file in your workspace or user profile.

**TIP:** Type `/agents` in the chat input to quickly open the **Configure Custom Agents** menu.
...
When you use `tools` in both a custom agent and a prompt file, the prompt file's tools take precedence. For the full priority order, see [Tool list priority](prompt-files.md#tool-list-priority) in the prompt files documentation.
...
**TIP:** Use the chat customization diagnostics view to see all loaded custom agents, prompt files, instruction files, and skills along with any errors. Right-click in the Chat view and select **Diagnostics**. Learn more about [troubleshooting AI in VS Code](../agents/agent-troubleshooting/troubleshooting.md).
...
* [Create reusable prompt files](prompt-files.md)
