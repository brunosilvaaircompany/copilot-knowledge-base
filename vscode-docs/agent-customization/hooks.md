...
For background on how hooks fit into the AI customization framework, see [Customization concepts](../agents/concepts/customization.md).

This article explains how to configure and use hooks in {% data variables.product.prodname_vscode_shortname %}.

**NOTE:** Agent hooks are currently in Preview. The configuration format and behavior might change in future releases.

**IMPORTANT:** Your organization might have disabled the use of hooks in {% data variables.product.prodname_vscode_shortname %}. Contact your admin for more information. See [enterprise policies](https://code.visualstudio.com/docs/enterprise/policies) for details.
...
|------------|---------------|------------------|
| [`SessionStart`](../agents/reference/hooks-reference.md#sessionstart) | User submits the first prompt of a new session | Initialize resources, log session start, validate project state |
| [`UserPromptSubmit`](../agents/reference/hooks-reference.md#userpromptsubmit) | User submits a prompt | Audit user requests, inject system context |
| [`PreToolUse`](../agents/reference/hooks-reference.md#pretooluse) | Before agent invokes any tool | Block dangerous operations, require approval, modify tool input |
| [`PostToolUse`](../agents/reference/hooks-reference.md#posttooluse) | After tool completes successfully | Run formatters, log results, trigger follow-up actions |
| [`PreCompact`](../agents/reference/hooks-reference.md#precompact) | Before conversation context is compacted | Export important context, save state before truncation |
| [`SubagentStart`](../agents/reference/hooks-reference.md#subagentstart) | Subagent is spawned | Track nested agent usage, initialize subagent resources |
| [`SubagentStop`](../agents/reference/hooks-reference.md#subagentstop) | Subagent completes | Aggregate results, cleanup subagent resources |
| [`Stop`](../agents/reference/hooks-reference.md#stop) | Agent session ends | Generate reports, cleanup resources, send notifications |

For the full input and output schema of each event, see the [Hooks reference](../agents/reference/hooks-reference.md).
...
**TIP:** In a monorepo, enable `setting(chat.useCustomizationsInParentRepositories)` to discover hooks from the parent repository root. Learn more about [parent repository discovery](overview.md#use-customizations-in-a-monorepo).
...
| Plugin | `hooks.json` or `hooks/hooks.json`, depending on the plugin format (see [Hooks in plugins](agent-plugins.md#hooks-in-plugins)) |
...
Each hook entry must specify `type: "command"` and a command to run. You can also configure a working directory (`cwd`), environment variables (`env`), a `timeout`, and OS-specific overrides (`windows`, `linux`, `osx`). For the full list of properties, see the [Hook command properties reference](../agents/reference/hooks-reference.md#hook-command-properties).
...
**NOTE:** Agent-scoped hooks are currently in preview.

You can define hooks directly in a [custom agent's](custom-agents.md) YAML frontmatter. Agent-scoped hooks only run when that custom agent is active, either selected by the user or invoked as a subagent. Agent-scoped hooks run in addition to any workspace or user-level hooks configured for the same event.
...
Each hook event provides its own input fields and supports event-specific output. For the full input and output schema of every event, including `PreToolUse`, `PostToolUse`, `SessionStart`, `Stop`, and more, see the [Hooks reference](../agents/reference/hooks-reference.md).
...
**TIP:** You can also run the **Developer: Show Agent Debug Logs** command to view hook input and output in the agent debug logs.
...
* [Use tools with agents](../agents/run/tools.md) - Learn about tool approval and execution
* [Custom agents](custom-agents.md) - Create specialized agent configurations
* [Subagents](../agents/run/subagents.md) - Delegate tasks to context-isolated subagents
* [Security considerations](../agents/run/security.md) - Best practices for AI security in {% data variables.product.prodname_vscode_shortname %}
