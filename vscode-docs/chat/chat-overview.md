...
The interaction mechanics in this article apply to the {% data variables.copilot.chat_view %}, chat editor tabs, and the {% data variables.copilot.agents_window %}. To compare these interfaces with browser and terminal options, see [Ways to work with agents](../agents/overview.md#ways-to-work-with-agents).

For lightweight interactions, use [inline chat](inline-chat.md) to edit code or get terminal suggestions in place, or open [Quick Chat](inline-chat.md#use-quick-chat) for a temporary conversation.
...
For a hands-free conversation, start [Voice Mode](https://code.visualstudio.com/docs/configure/accessibility/voice#use-voice-mode) from the chat input. Voice Mode listens for follow-up requests and reads the agent's responses aloud. `feature(voice-mode)`
...
For common tasks, you can use slash commands as shortcuts for frequently used prompts or to invoke [agent skills](../agent-customization/agent-skills.md). Type `/` in the chat input to see all available commands.
...
**NOTE:** The `!` command is only available in [Agent Host](../agents/concepts/agent-host.md) sessions.
...
Stopping a request doesn't undo file edits, terminal commands, or other actions that already completed. To restore affected workspace files and chat history, [restore a checkpoint](../agents/run/review-code-edits.md#restore-a-checkpoint). Checkpoints don't reverse changes to external services.

For examples of when to steer or stop a request and how to write a focused correction, see [Get an agent back on track](../agents/guides/get-agent-back-on-track.md#redirect-a-running-request).
...
Learn more about [managing context for AI](copilot-chat-context.md).
...
**NOTE:** Light and dark color themes have separate backgrounds. Background choices are stored on the current machine and don't sync, while the image layout syncs across devices.
...
* **Write effective prompts**: be specific about what you want, reference relevant files and symbols, and use `/` commands for common tasks. Get inspired by [prompt examples](../agents/guides/prompt-examples.md) or review the full [prompt engineering guide](../agents/best-practices.md).

* **Customize the AI**: tailor the AI's behavior to your project by adding [custom instructions](../agent-customization/custom-instructions.md), creating reusable [prompt files](../agent-customization/prompt-files.md), or building [custom agents](../agent-customization/custom-agents.md) for specialized workflows. For example, create a "Code Reviewer" agent that provides feedback on code quality and adherence to your team's coding standards.
...
Use [Agent Logs and the Chat Debug view](../agents/agent-troubleshooting/chat-debug-view.md) to inspect what happens when you send a prompt. Agent Logs shows a chronological event log of tool calls, LLM requests, and prompt file discovery. The Chat Debug view shows the raw system prompt, user prompt, context, and tool payloads for each interaction. These tools are useful for understanding why the AI responded in a certain way or for troubleshooting unexpected results.
