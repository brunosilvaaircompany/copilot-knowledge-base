...
* `/every` — schedule a prompt to be sent **repeatedly**, either at a fixed interval or at a cadence determined by Copilot.
...
In an interactive Copilot CLI session, type `/every` followed by the prompt you want to be submitted. To use a fixed interval, include the interval before the prompt.
